...
class CartLogic
{
     ...
     * 修改选中状态
     */
    public static function changeStatus($id, $id_selected)
    {
        ...
        if(session('?user_info')){
            ...
            if ($id=='all'){
                //全部修改
                foreach ($data as &$v){
                    $v['is_selected']=$id_selected;
                }
                unset($v);
            }else{
                //修改一个
                $data[$id]['is_selected']=$id_selected;
            }
            //重新保存到cookie
            cookie('cart',$data,86400*7);
        }
